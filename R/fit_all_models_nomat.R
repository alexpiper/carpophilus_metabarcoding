...
bias_recipe <- 
  recipe(formula = error ~ 0 + taxon + fcid , data = splits$train[[1]]) %>% 
  step_string2factor(fcid, taxon) %>% 
...
prop_recipe <- recipe(formula =  expected.prop ~ 0 + observed.prop + taxon + fcid , data = splits$train[[1]]) %>% 
...
# write out fits_all
saveRDS(fits_all, "fits_all_nomat.rds")
