...
tb2 <- tibble(
  ...
  DNA = c(1.1, 1.2, 0.6, 0.8),
  Amplicons = c(1.6, 2.4, 1.1, 0.3),
  Libraries = c(1.65, 2.2,0.95, 0.32),
  Reads = c(1.45, 2.3, 1, 0.28),
  ASVs = c(1.5, 2.5, 0.95, 0.34)
) %>%
...
gg.multi <- tb2 %>%
  ...
  scale_x_discrete(limits = c("Individuals","DNA","Amplicons","Libraries","Reads","ASVs")) +
...
tb <- tibble(
  ...
  Bias = c(1, 18, 6, 4),
  S1 = c(1, 1, 1, 1),
  S2 = c(1, 1/15, 4/15, 2/15),
  S3 = c(1, 4/15, 1/15, 2/15),
) 
...
p2.ratios <- ggplot(tb, aes(x = Sample, y = Abundance, color = Taxon)) +
  geom_path(aes(group = Taxon),size=1) +
  geom_point(size=2) +
  facet_wrap(~Type) +
  scale_y_log10() +
