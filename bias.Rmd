...
prop_recipe_separate <- 
  recipe(formula = observed.prop ~ 0 + expected.prop , data = joint_train) %>% 
  step_zv(all_predictors())
...
fit_lm <- joint_train %>%
  ...
  mutate(model_obj = map(train, ~fit(lm_spec, observed.prop ~ 0 + expected.prop, data = .)),
    ...
    model_obj = purrr::map2(train, error_type, function(x,y ){
      ...
      if (!y =="error_prop"){
        fit(lm_workflow_rat, data=x)
         ...
         group_by(taxon) %>%
         nest() %>%
         mutate(model_obj = purrr::map(data, ~fit(lm_workflow_prop, data=.)),
                coef_info = purrr::map(model_obj, ~broom::tidy(.x))) %>%
         unnest(coef_info) %>%
         filter(term =="expected.prop") %>%
         dplyr::select(taxon, estimate)
      }
    }),
