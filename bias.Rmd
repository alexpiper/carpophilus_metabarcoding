...
# Plot tree, with taxon names an a
...
fits_all <- splits %>%
  filter(error_type %in% c("error_prop"), model_type %in% c("svm", "lm", "uncorrected")) %>%
  mutate(
...
breakpoint_fit <- fits %>%
  ...
  group_by(model) %>%
  nest() %>%
  mutate(fits = purrr::map(data, ~chngptm(formula.1=rsq~1, formula.2=~expected.prop,
                                   type="upperhing", family="gaussian", ci.bootstrap.size	
=1000, est.method	= "fastgrid", data = .)),
...
# Get SE of breakpoints - Only seems to work for segmented?
breakpoint_fit %>% 
...
gg.quant_limit <-  fits %>%
    filter(error_type == "Proportions", model == "Random Forest") %>%
  ggplot(aes(x = expected.prop, y = rsq, colour=taxon)) +
...
# Subset to Nitidulidae
niti <- names(seqs) %>%
  str_split_fixed(";", n=8) %>%
  as_tibble() %>%
  filter(V6 == "Nitidulidae") %>%
  tidyr::unite(everything(),col="out", sep=";") %>%
  pull(out)

seqs <- seqs[names(seqs) %in% niti]

# Add primers to seqs
primers.dna <- insect::char2dna(c("GGDACWGGWTGAACWGTWTAYCCHCC", rc("GTRATWGCHCCDGCTARWACWGG")))
names(primers.dna) <- c("fwhF2", "fwhR2n")

#Join to seqs
seqs <- c(seqs, primers.dna)

# Trim seqs to Fprimer bind region and get primer edit distance
aligned <- taxreturn::map_to_model(seqs, coi_model, minscore = 0, shave= TRUE, pad=TRUE, check_indels=FALSE, maxNs=Inf, cores=1, quiet=FALSE, extra = "fill")
...
Fprimerdist <- Biostrings::subseq(aligned, start=primers$Fstart[1], end = primers$Fend[1]) %>%
  ...
  tidyr::separate(taxon, into=c("Acc" ,"Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species"), sep=";")

#Get reverse dist
Rprimerdist <- Biostrings::subseq(aligned[lengths(aligned) > primers$Rend[1]],
