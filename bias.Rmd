...
joint <- sam %>%
  ...
  filter(!sample_id=="HLVKYDMXX_CML6") #Bad sample
...
model_rmse <- fits %>%
  ...
  rmse(truth = logit(calibrated), estimate = logit(expected.prop)) %>%
...
gg.train_preds <- fits %>%
  ...
  ggplot(aes(logit(calibrated), logit(expected.prop), fill = material_type)) +
  geom_point(alpha = 0.5, shape=21, colour="black") +
  geom_abline(lty = 2, colour = "gray80", size = 1) +
    geom_text(aes(x=-10, y=2, label=paste0("RMSE: ",round(RMSE, 2))),check_overlap = TRUE, inherit.aes = FALSE, hjust = 0)+
#  geom_text(aes(x=-10, y=1, label=paste0("A.dist: ",round(Adist, 2))),check_overlap = TRUE, inherit.aes = FALSE, hjust = 0)+
 # coord_fixed() + 
  ...
  theme(panel.grid = element_line(size = rel(1)),
    ...
    labs(x = "Calibrated proportions (log-odds)", 
...
gg.train_abs <- abs_fits %>%
  ...
  mutate(model = model %>%
           ...
           str_replace("lasso", "LASSO Reg (Ratios)")%>%
           str_replace("complm", "Linear Reg (Ratios)")) %>%
  filter(train == "Training set") %>%
  filter(error_type %in% c("error_clr", "error_alr",  "error_prop", "error_abs")) %>%
  mutate(model = factor(model),
         model = fct_reorder(model, -RMSE)) %>%
