...
gg.err_var <- error_variation %>%
  mutate(taxon = taxon %>% str_replace("_", " ")) %>%
  ggplot(aes(x = type, y = error_prop, fill=taxon, group=type))+
  ...
  scale_fill_brewer(palette = "Paired")+
  facet_grid(~taxon) +
  scale_y_continuous(trans=pseudo_log_trans(1e-1), breaks=c(0, 1, 3, 5, 10))+
  base_theme +
  theme(
...
#Save figure
pdf(file="fig/supplementary/error_consistency.pdf", width = 10, height = 8 , paper="a4r")
  plot(Fig2)
try(dev.off(), silent=TRUE)
...
fits_all <- splits %>%
  filter(error_type %in% c("error_clr", "error_alr", "error_abs", "error_prop"), model_type %in% c("metacal", "lm", "uncorrected")) %>%
  mutate(
...
test_splits <- splits %>%
  filter(error_type %in% c("error_prop_logit"), model_type %in% c("lasso")) 

train <- test_splits$train[[1]]
test <- test_splits$test[[1]]
cv <- test_splits$cv[[1]]
error_type <- test_splits$error_type[[1]]
model_type <- test_splits$model_type[[1]]
...
fits_all <- splits %>%
  ...
  dplyr::slice(1) %>%
  ungroup() %>%
  filter(model_type == "lasso") %>% # error_type == "error_abs_log"
  #  filter(error_type == c("error_clr", "error_alr", "error_abs", "error_prop")) %>%
    mutate(
        fits = purrr::pmap(list(train, test, cv, error_type, model_type), modelling_func)
    ) %>% 
    unnest(fits)
...
```{r evaluate fits}
# Read back in fits_all
fits_all <- readRDS("fits_all.rds")
...
fits <- all_fits_train %>%
  ...
  dplyr::select(model = model_type,seed, taxon, material_type, sample_name, sample_id, fcid, expected, expected.prop ,observed, observed.prop, predicted, estimated, train, calibrated, residual,  error_type ,sample_size) %>%
  mutate(model = model %>%
...
abs_fits <- all_fits_train %>%
  ...
  bind_rows(all_fits_test %>%
    mutate(train = "Test set")) %>%
  dplyr::select(model = model_type, seed, taxon, material_type, sample_name, sample_id, fcid, expected, expected.prop ,observed, observed.prop, predicted, estimated, calibrated, train, error_type ,sample_size) %>%
...
library(ggpubr)
compare_means(RMSE ~ model,  data = model_rmse, ref.group = "Uncorrected", group.by = c("train", "error_type", "type"),
              method = "t.test")

# ANOVA
compare_means(RMSE ~ model,  data = model_rmse %>% filter(!model == "Uncorrected"), group.by = c("train", "error_type", "type"), method = "anova")
...
gg.rmse <- model_rmse  %>%
  ...
  filter(!(model == "metacal"& !error_type %in% c("error_alr", "error_clr"))) %>%
 # dplyr::filter(error_type %in% c("error_prop", "error_abs", "error_clr", "error_alr")) %>%
  mutate(
    error_type = error_type %>%
      str_remove("error_") %>%
      str_replace("prop$", "Proportions") %>%
      str_replace("prop_log$", "Log(Proportions)")%>%
      str_replace("prop_logit$", "Logit(Proportions)")%>%
      str_replace("alr$", "ALR")%>%
      str_replace("clr$", "CLR")%>%
      str_replace("abs$", "Absolute")%>%
      str_replace("abs_log$", "Log(Absolute)") %>%
      factor(levels = c(
        "Proportions",
        "Log(Proportions)",
        "Logit(Proportions)",
  ...
  facet_grid(error_type~type, drop = TRUE)+ 
  coord_flip() +
  base_theme +
  theme(legend.position = "none",
...
pdf(file="fig/Fig2_model_comparison.pdf", width = 8, height = 11 , paper="a4")
  plot(gg.rmse)
try(dev.off(), silent=TRUE)
...
gg.rf_taxon_res <- fits %>%
  filter(error_type == "error_abs", model == "Random Forest") %>%
...
# Look at residuals by relative abundance in community
fits %>%
    filter(error_type == "error_abs", model == "Random Forest") %>%
  ggplot(aes(x = expected.prop, y = residual, colour=taxon)) +
  geom_point(position=position_jitter(width = 0.01, height=0), alpha=0.5) +
...
breakpoint_fit <- fits %>%
  ...
  group_by(model) %>%
  nest() %>%
  mutate(fits = purrr::map(data, ~chngptm(formula.1=residual~1, formula.2=~expected.prop,
...
slope_coefs <- breakpoint_fit %>%
  unnest(coef_info) %>%
  dplyr::select(model, term, est, std_err = `Std. Error*`,
...
dat_pred <- breakpoint_fit %>%
  ...
  left_join(slope_coefs %>%
              dplyr::select(model,term, signif, est) %>%
              pivot_wider(names_from=term, values_from=c(signif, est))) 


break_annot <- dat_pred %>% 
  filter(expected.prop == breakpoint) %>%
  dplyr::slice(1) 

gg.quant_limit <- fits %>%
    filter(error_type == "error_abs", model == "Random Forest") %>%
  ggplot(aes(x = expected.prop, y = residual, colour=taxon)) +
  geom_point(position=position_jitter(width = 0.01, height=0), alpha=0.2) +
  geom_line(data=dat_pred[dat_pred$expected.prop < dat_pred$breakpoint,],
             aes(x=expected.prop, y=preds),  color = 'blue', size=0.7, inherit.aes = FALSE) +
  geom_line(data=dat_pred[dat_pred$expected.prop >= dat_pred$breakpoint,],
            aes(x=expected.prop, y=preds),  color = 'blue', size=0.7, inherit.aes = FALSE) +
  geom_point(data=break_annot, aes(x=expected.prop, y=preds),size=2,  color = 'blue', inherit.aes = FALSE) +
  geom_vline(data=dat_pred, aes(xintercept = breakpoint), colour="black", linetype = "dashed") +
  geom_text(data = break_annot, aes(x = expected.prop, y = -0.35, label=scales::percent(expected.prop,accuracy = 0.02)),
            colour="black", nudge_x = .1)+
  scale_x_continuous(labels = scales::percent)+
  facet_grid(~model) +
  scale_colour_brewer(palette="Paired")+
  base_theme +
  labs(x = "Relative abundance in community",
       y = "Residual error")

gg.quant_limit

gg.rf_taxon_res + gg.quant_limit + plot_layout(widths = c(2,1))
```
...
gg.train_preds <- fits %>%
  ...
  scale_x_continuous(trans = scales::pseudo_log_trans(1e-2), labels=scales::percent,
                       breaks=c(0.025, 0.05, 0.1, 0.25, 0.5, 1), limits=c(0,1), expand = c(0,0))+
...
fit_jacknifes <- readRDS("output/jacknifes.rds")
...
# Try doing this by taxon!
fig3a <- gg.quant_limit /(gg.rf_taxon_res + theme(axis.title.x = element_blank())) 
fig3 <- fig3a - gg.jacknife_box + plot_layout(widths = c(2,0.8)) + plot_annotation(tag_levels = "A")

fig3

#Save figure
pdf(file="fig/Fig3_residual_summary.pdf", width = 11, height = 8 , paper="a4r")
  plot(fig3)
try(dev.off(), silent=TRUE)
