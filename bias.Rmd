...
Think something is up with the way i do this.

Need to split with full sample IDs so we can reclose after calibrating
...
Estimate bias using a simple linear model on proportions 
...
This approach treats the abundances and errors as compositional vectors
...
fits <- all_fits_train %>%
  ...
  mutate(model2 = model2 %>%
           ...
           paste0(model) %>%
           str_remove("NA")) %>%
  dplyr::select(model=model2, taxon, material_type, sample_name, sample_id, fcid, expected, observed, predicted, train, observed.prop) 

# missing the predicted
