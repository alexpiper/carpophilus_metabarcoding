...
train_cv <-  vfold_cv(joint_train, v = 5, strata = material_type )
...
fit_lm <- joint_train %>%
  filter(expected > 0) %>%
  group_by(taxon, material_type, fcid) %>%
  nest() %>%
...
fit_compl <- joint_train %>%
  ...
  mutate(model_obj = purrr::map(train, ~fit(lm_workflow, data=.)),
         pred_train = purrr::map2(model_obj, train, function(.model, .data) predict(.model, .data)),
         pred_test = purrr::map2(model_obj, test, function(.model, .data) predict(.model, .data)),
    )

#results
compl_results_train <- fit_compl %>%
  unnest(pred_train, train) %>%
  dplyr::select(!where(is.list))%>%
  mutate(estimated = case_when(
    error_type %in% c("error_alr", "error_clr", "error_centre", "error_abs_centre") ~ exp(.pred),
    TRUE ~ .pred
  ),
...
glmnet_tune <- tune_grid(glmnet_workflow, resamples =train_cv, grid = glmnet_grid)
autoplot(glmnet_tune , metric = "rmse") +
  scale_x_log10()

best_lambda <- glmnet_tune %>%
  select_best("rmse")

glmnet_workflow <- finalize_workflow(glmnet_workflow, best_lambda)
...
fit_rf <- joint_train %>%
  ...
  nest() %>%
  dplyr::rename(test=data)) %>%
  mutate(model_obj = map(train, ~fit(ranger_workflow, data=.)),
         pred_train = map2(model_obj, train, function(.model, .data) predict(.model, .data)),
         pred_test = map2(model_obj, test, function(.model, .data) predict(.model, .data)),
    )
...
fit_xg <- joint_train %>%
  ...
  nest() %>%
  dplyr::rename(test=data)) %>%
  mutate(model_obj = map(train, ~fit(xgboost_workflow, data=.)),
         pred_train = map2(model_obj, train, function(.model, .data) predict(.model, .data)),
         pred_test = map2(model_obj, test, function(.model, .data) predict(.model, .data)),
    )
...
fit_keras <- joint_train %>%
  ...
  nest() %>%
  dplyr::rename(test=data)) %>%
  mutate(model_obj = map(train, ~fit(keras_workflow, data=.)),
         pred_train = map2(model_obj, train, function(.model, .data) predict(.model, .data)),
         pred_test = map2(model_obj, test, function(.model, .data) predict(.model, .data)),
    )
...
test1 <- metacal_spec(tests$data[[1]], denom="Carpophilus_hemipterus")
test2 <- metacal_spec(tests$data[[5]], denom="Carpophilus_hemipterus")
all(round(test1$.pred,4) == round(test2$.pred,4))


#Test CLR on on original erorrs is equivalent to the pre-transformed CLR error
test3 <- metacal_spec(tests$data[[1]])
test4 <- metacal_spec(tests$data[[6]])
all(round(test3$.pred,4) == round(test4$.pred,4))

# Fit metacal
fit_metacal <- joint_train %>%
  filter(expected > 0) %>%
  group_by(error_type, material_type, fcid) %>%
  nest() %>%
  dplyr::rename(train=data)%>%
  left_join(joint_test %>%
    filter(expected > 0) %>%
  ...
  mutate(denom = case_when(
   ...
   mutate(pred_train = purrr::map(train, metacal_spec,denom=denom),
          pred_test = purrr::map(test, metacal_spec,denom=denom))
...
metacal_results_train <-  fit_metacal%>%
  ...
  left_join(fit_metacal %>% 
              unnest(pred_train) %>% 
              dplyr::select(error_type, taxon, .pred))%>%
  dplyr::filter(expected > 0) %>%
  mutate(
      predicted = expected.prop * .pred,
      estimated = .pred
    )  %>%
  group_by(sample_id) %>%
  mutate_at(vars(predicted), ~ . / sum(., na.rm=TRUE) ) %>%
...
fits <- all_fits_train %>%
  ...
  mutate(model = model %>%
           str_remove("_results.*$") %>%
           str_remove("NA")) %>%
  dplyr::select(model, taxon, material_type, sample_name, sample_id, fcid, expected, expected.prop ,observed, observed.prop, predicted, estimated, train, error_type ) 
...
gg.train_preds <- fits %>%
  ...
  mutate(model = model %>%
           ...
           str_replace("compl", "Linear Reg (Ratios)")) %>%
  filter(train == "Training set", error_type == "error_clr") %>%
...
# Fit final model with boostrapping
